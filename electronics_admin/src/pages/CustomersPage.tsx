...
const CustomersPage = () => {
  ...
  const fetchCustomers = async () => {
    let url = `${SETTINGS.URL_API}/v1/customers?`;
    if (name) {
      url += `keyword=${name}&`;
    }
    if (email) {
  ...
  const getAllCustomers = useQuery({
    queryKey: ["customers", page, name, email, phone],
    queryFn: fetchCustomers,
  });

  const queryClient = useQueryClient();
  const fetchDeleleCustomers = async (id: string) => {
    const url = `${SETTINGS.URL_API}/v1/customers/${id}`;
    const res = await axiosClient.delete(url);
    return res.data.data;
  };
  const deleteCustomer = useMutation({
    mutationFn: fetchDeleleCustomers,
    onSuccess: () => {
  ...
  const onFinishSearch = async (values: TFilter) => {
    ...
    navigate(`/customers${queryString ? `?${queryString}` : ""}`);
  };
  ...
  return (
    <>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Electronics - Khách hàng </title>
              ...
              <div className="w-full overflow-x-auto">
                ...
                <table className="w-full whitespace-no-wrap">
                  ...
                  <tbody className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    {getAllCustomers?.data?.customers_list.length > 0 ? (
                      getAllCustomers?.data?.customers_list.map(
                        (item: TCustomer, i: number) => {
                          return (
                            <tr
                              key={i}
              ...
              <div className="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
                <span className="flex col-span-12 mt-2 m-auto">
                  <nav aria-label="Table navigation">
                    {getAllCustomers?.data?.pagination.totalRecords >
                      getAllCustomers?.data?.pagination.limit && (
                      <Pagination
                        ...
                        total={
                          getAllCustomers?.data?.pagination.totalRecords || 0
                        }
                        pageSize={getAllCustomers?.data?.pagination.limit || 10}
                      />
                    )}
                  </nav>
                </span>
